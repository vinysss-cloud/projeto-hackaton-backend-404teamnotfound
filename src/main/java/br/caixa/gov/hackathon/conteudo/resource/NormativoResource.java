...
public class NormativoResource {
    ...
    public ApiResponse<PaginacaoDTOs.PaginaResponse<PortalDTOs.ConteudoResponse>> listar(
        ...
        List<PortalDTOs.ConteudoResponse> itens = conteudoRepository.listarNormativos(paginaSegura, tamanhoSeguro)
                .stream()
                .map(PortalDTOs.ConteudoResponse::from)
                .toList();
        return ApiResponse.ok("Normativos carregados com sucesso.",
                PaginacaoDTOs.PaginaResponse.of(itens, conteudoRepository.contarNormativos(), paginaSegura, tamanhoSeguro, "ordem", "asc"));
    }
    ...
    public ApiResponse<PortalDTOs.ConteudoResponse> buscar(@PathParam("id") @Positive(message = "ID deve ser maior que zero.") Long id) {
        ...
        if (conteudo == null || !"NORMATIVO".equalsIgnoreCase(conteudo.tipo)) {
            throw new NotFoundException("Normativo não encontrado.");
        }
        return ApiResponse.ok("Normativo localizado com sucesso.", PortalDTOs.ConteudoResponse.from(conteudo));
    }
}
